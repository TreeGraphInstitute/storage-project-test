...
for domain in "${domains[@]}"; do
    if ! nslookup "$domain" > /dev/null 2>&1; then
        # A hack to make network setup
        exit 0
    fi
done

# 运行命令并捕获输出
hex_epoch_number=$(curl -s -X POST --data '{"jsonrpc":"2.0","method":"cfx_getBlockByEpochNumber","params":["latest_state", false],"id":1}' -H "Content-Type: application/json" localhost:12539 | jq -r ".result.epochNumber")
